...
export default function ProjectList({ initialProjects }: { initialProjects?: Project[] }) {
  ...
  const members = useQuery({ queryKey: ["organization-members", selectedOrganization, token], queryFn: () => api<Member[]>(`/organizations/${selectedOrganization}/members`, token), enabled: !!token && !!selectedOrganization });
  ...
  async function addOrganizationMember(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget);
    try {
      await api(`/organizations/${selectedOrganization}/members`, token, { method: "POST", body: JSON.stringify({ email: form.get("email"), role: form.get("role") }) });
      event.currentTarget.reset();
      client.invalidateQueries({ queryKey: ["organization-members", selectedOrganization, token] });
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to add teammate"); }
  }
  ...
  return <>
    ...
    <h2>Organization teammates</h2>
    <select value={selectedOrganization} onChange={(event) => setSelectedOrganization(event.target.value)}>
      <option value="">Select organization</option>
      {organizations.data?.map((organization) => <option value={organization.id} key={organization.id}>{organization.name}</option>)}
    </select>
    {selectedOrganization && <>
      <div className="stack">{members.data?.map((member) => <div className="card" key={member.user_id}><strong>{member.name}</strong><div className="muted">{member.email} · {member.role}</div></div>)}</div>
      <form onSubmit={addOrganizationMember}>
        <input name="email" type="email" placeholder="Registered teammate email" required />
        ...
        <button>Add or update teammate</button>
      </form>
    </>}
